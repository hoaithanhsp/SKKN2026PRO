...
export const ConfigForm: React.FC<ConfigFormProps> = ({ onStart, isLoading }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
      <div className="bg-gradient-to-r from-orange-600 to-indigo-600 p-8 text-white">
        <h2 className="text-3xl font-bold flex items-center gap-3">
          <BookOpen className="w-8 h-8" />
          Tạo Đề Luyện Thi
        </h2>
        <p className="mt-2 text-orange-100">Cấu trúc chuẩn Bộ Giáo Dục 2025</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all text-lg"
            required
            disabled={isLoading}
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div 
              ...
              className={`cursor-pointer p-4 rounded-xl border-2 transition-all ${!hasEssay ? 'border-orange-500 bg-orange-50 ring-1 ring-orange-500' : 'border-slate-200 hover:border-slate-300'}`}
            >
              <div className="flex items-start gap-3">
                { !hasEssay ? <CheckCircle2 className="w-6 h-6 text-orange-600 flex-shrink-0" /> : <Circle className="w-6 h-6 text-slate-300 flex-shrink-0" /> }
                <div>
